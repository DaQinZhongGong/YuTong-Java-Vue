...
/**
 * GA2-L185 CT-createDictType 契约测试。
 *
 * <p>设计来源: 58-后端API逐接口任务清单 (CT-createDictType 验收标准)
 * <p>策略来源: operation-policies.yaml createDictType → uniqueCreate profile (database-unique-constraint)
 * <p>契约来源: openapi.yaml POST /api/v1/dict-types (x-permission: system:dict:add, x-error-codes: [SYS-409004])
 *
 * <p>覆盖 58 号文档通用覆盖矩阵 9 类场景:
 * <ol>
 *   <li>CT-1 成功创建 (200, code=0, 返回 DictType + traceId)</li>
 *   <li>CT-2 认证失败 (Mock 模式限制, @Disabled)</li>
 *   <li>CT-3 权限拒绝 (viewer 无 system:dict:add, 403 AUTH-403001)</li>
 *   <li>CT-4 参数校验失败 (dictName 空/status 非法/dictType 缺失, 400)</li>
 *   <li>CT-5 重复编码 (409 SYS-409004)</li>
 *   <li>CT-6 标准错误信封 (code/message/traceId 字段齐全)</li>
 *   <li>CT-7 traceId 透传 (响应头 + 响应体)</li>
 *   <li>CT-8 幂等 (重复提交同 dictType, 第二次 409)</li>
 *   <li>CT-9 审计落库 (sys_operation_log 有 CREATE 记录)</li>
 * </ol>
 *
 * <p>数据隔离: 每个用例使用唯一 dictType (UUID 后缀), @AfterEach 清理创建的数据。
 * 不使用 @Transactional 以便 CT-9 审计验证能查询 sys_operation_log。
 *
 * <p>注意: Dict 为系统模块, 仅 admin (通配权限 *) 有 system:dict:add; biz/viewer 均无该权限。
 */
@DisplayName("CT-createDictType: POST /api/v1/dict-types 契约测试")
class CTCreateDictTypeTest extends AbstractContractTest {
    ...
    void cleanupTestData() {
        ...
        // 清理本测试创建的字典类型数据 (物理删除, 避免逻辑删除残留影响唯一索引)
        for (String id : createdIds) {
    ...
    private DictType buildValidDictType() {
        ...
        t.setDictType("CT-L185-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase());
        t.setDictName("CT测试字典类型");
        t.setStatus("ENABLED");
        t.setSortNo(0);
        return t;
    }

    /** 从成功响应中提取字典类型 ID 并记录到清理集合。 */
    ...
    @Test
    @DisplayName("CT-1: 成功创建字典类型 (admin 用户, 200, code=0, 返回 id/version/traceId)")
    void testCreateDictTypeSuccess() throws Exception {
        ...
        assertEquals(request.getDictName(), data.get("dictName").asText());
    ...
    void testCreateDictTypeUnauthenticated() {
    ...
    void testCreateDictTypeForbidden() throws Exception {
        DictType request = buildValidDictType();
    ...
    @Test
    @DisplayName("CT-4a: 参数校验失败 - dictName 为空 (400)")
    void testCreateDictTypeValidationBlankName() throws Exception {
        DictType request = buildValidDictType();
        request.setDictName(""); // @NotBlank 校验
    ...
    void testCreateDictTypeValidationInvalidStatus() throws Exception {
        DictType request = buildValidDictType();
    ...
    @Test
    @DisplayName("CT-4c: 参数校验失败 - dictType 缺失 (400)")
    void testCreateDictTypeValidationMissingCode() throws Exception {
        DictType request = buildValidDictType();
        request.setDictType(null); // @NotBlank 校验

        MvcResult result = performPost(API_PATH, request, mockUser("admin"));

        assertEquals(400, result.getResponse().getStatus(), "dictType 缺失, HTTP 应为 400");
        ResultNode node = parseResult(result);
        assertNotEquals("0", node.code(), "校验失败不应返回 code=0");
        assertTraceIdPresent(result);
    }
    ...
    @Test
    @DisplayName("CT-5: 重复编码 (同租户同 dictType, 409 SYS-409004)")
    void testCreateDictTypeDuplicateCode() throws Exception {
        DictType request = buildValidDictType();
        ...
        // 第二次创建同 dictType: 应失败 409 SYS-409004
    ...
    void testCreateDictTypeErrorEnvelopeStructure() throws Exception {
        DictType request = buildValidDictType();
        request.setDictName(""); // 触发 400
    ...
    void testCreateDictTypeTraceIdPropagation() throws Exception {
        DictType request = buildValidDictType();
    ...
    // ===== CT-8: 幂等 (重复提交同 dictType 触发唯一约束) =====

    @Test
    @DisplayName("CT-8: 幂等-重复提交同 dictType (uniqueCreate 策略, 第二次 409 SYS-409004)")
    void testCreateDictTypeIdempotency() throws Exception {
        DictType request = buildValidDictType();
        ...
        // 第二次同 dictType: 幂等校验 → 409 (uniqueCreate 策略: database-unique-constraint)
        ...
        // 验证不会产生重复数据: 查询同 dictType 只有一条
        Integer count = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM sys_dict_type WHERE dict_type = ? AND deleted = false",
                Integer.class, request.getDictType());
        assertNotNull(count);
        assertEquals(1, count, "幂等校验后同 dictType 应只有 1 条记录");
        assertEquals(firstId, jdbcTemplate.queryForObject(
                "SELECT id FROM sys_dict_type WHERE dict_type = ? AND deleted = false",
                String.class, request.getDictType()), "应保留第一次创建的记录 id");
    }
    ...
    void testCreateDictTypeAuditLog() throws Exception {
        DictType request = buildValidDictType();

        MvcResult result = performPost(API_PATH, request, mockUser("admin"));
        assertEquals(200, result.getResponse().getStatus());
        assertSuccess(result);
        String dictTypeId = extractAndTrackId(result);

        // 查询 sys_operation_log 表验证审计记录
        String traceId = parseResult(result).traceId();
        Integer auditCount = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM sys_operation_log " +
                        "WHERE biz_type = 'sys_dict_type' AND operation_type = 'CREATE' " +
                        "AND trace_id = ?",
