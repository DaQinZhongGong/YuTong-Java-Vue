...
/**
 * CT-archiveBizRequest 契约测试。
 *
 * <p>设计来源: 58-后端API逐接口任务清单 (CT-archiveBizRequest 验收标准)
 * <p>策略来源: operation-policies.yaml archiveBizRequest → workflowAction profile
 * <p>契约来源: openapi.yaml POST /api/v1/biz-requests/{id}/archive
 *              (x-permission: biz:request:archive, x-error-codes: [BIZ-404001, BIZ-409001, SYS-409001])
 *
 * <p>状态机: APPROVED → ARCHIVED (archive)
 *   注意: archive 状态冲突错误码复用 BIZ-409001 (submitNotAllowed)。
 *
 * <p>覆盖矩阵:
 * <ol>
 *   <li>CT-1 成功归档 (biz 创建+提交, admin 审批+归档, APPROVED→ARCHIVED, 200, code=0)</li>
 *   <li>CT-2 认证失败 (Mock 模式限制, @Disabled)</li>
 *   <li>CT-3 权限拒绝 (viewer 无 biz:request:archive, 403 AUTH-403001)</li>
 *   <li>CT-4 申请单不存在 (404 BIZ-404001)</li>
 *   <li>CT-5 状态冲突 (DRAFT→archive 不允许, 409 BIZ-409001)</li>
 *   <li>CT-6 版本冲突 (stale version, 409 SYS-409001)</li>
 *   <li>CT-7 审计落库 (@Auditable ARCHIVE → sys_operation_log 有记录)</li>
 ...
 *   <li>admin 用户: 执行审批+归档 (有 "*" 通配权限, ALL 数据范围)</li>
 *   <li>approver 用户: 有 biz:request:archive 但 CUSTOM 空白名单 → 数据范围拒绝</li>
 *   <li>viewer 用户: 无 biz:request:archive → 权限拒绝</li>
 * </ul>
 */
@DisplayName("CT-archiveBizRequest: POST /api/v1/biz-requests/{id}/archive 契约测试")
class CTArchiveBizRequestTest extends AbstractContractTest {
    ...
    private SaveBizRequestRequest buildValidRequest() {
        ...
        req.setTitle("CT-ARCHIVE-" + UUID.randomUUID().toString().substring(0, 8));
        req.setCustomerId("01CTCUSTOMER0000000000000001");
        req.setCustomerNameSnapshot("CT归档测试客户");
        req.setApplyReason("CT归档测试申请原因");
        return req;
    }
    ...
    /** 审批通过申请单 (admin 用户, 指定 version)。 */
    private void approveRequest(String id, int version, HttpHeaders headers) throws Exception {
        Map<String, Object> body = new HashMap<>();
        body.put("opinion", "同意");
        body.put("version", version);
        body.put("idempotencyKey", "approve-" + UUID.randomUUID());
        MvcResult result = performPost(API_PATH + "/" + id + "/approve", body, headers);
    ...
    private String createSubmittedAndApprove() throws Exception {
        ...
        approveRequest(id, version, adminHeaders);
        return id;
    }

    /** 构造归档请求体。 */
    private Map<String, Object> buildArchiveBody(Integer version) {
        Map<String, Object> body = new HashMap<>();
        body.put("version", version);
        body.put("idempotencyKey", "archive-" + UUID.randomUUID());
        return body;
    }
    ...
    @Test
    @DisplayName("CT-1: 成功归档 (biz 创建+提交, admin 审批+归档, APPROVED→ARCHIVED, 200, code=0)")
    void testArchiveBizRequestSuccess() throws Exception {
        String id = createSubmittedAndApprove();

        MvcResult result = performPost(API_PATH + "/" + id + "/archive", buildArchiveBody(null), mockUser("admin"));
        ...
        assertEquals("ARCHIVED", data.get("requestStatus").asText(), "归档后状态应为 ARCHIVED");
    }
    ...
    void testArchiveBizRequestUnauthenticated() {
    ...
    @Test
    @DisplayName("CT-3: 权限拒绝 (viewer 无 biz:request:archive, 403 AUTH-403001)")
    void testArchiveBizRequestForbidden() throws Exception {
        String id = createSubmittedAndApprove();

        MvcResult result = performPost(API_PATH + "/" + id + "/archive", buildArchiveBody(null), mockUser("viewer"));

        assertEquals(403, result.getResponse().getStatus(), "viewer 无归档权限, HTTP 应为 403");
    ...
    void testArchiveBizRequestNotFound() throws Exception {
        String nonExistentId = "01NOTEXIST00000000000000CT";

        MvcResult result = performPost(API_PATH + "/" + nonExistentId + "/archive", buildArchiveBody(null), mockUser("admin"));
    ...
    @Test
    @DisplayName("CT-5: 状态冲突 (DRAFT→archive 不允许, 409 BIZ-409001)")
    void testArchiveBizRequestStatusConflict() throws Exception {
        // 创建草稿但不提交 → 状态为 DRAFT
        String id = createRequestAndTrack();

        // 从 DRAFT 归档 → 不允许 (ARCHIVE 仅允许 APPROVED; DRAFT 非幂等目标 ARCHIVED)
        MvcResult result = performPost(API_PATH + "/" + id + "/archive", buildArchiveBody(0), mockUser("admin"));
    ...
    void testArchiveBizRequestVersionConflict() throws Exception {
        // 创建+提交+审批通过 → version 从 0 递增到 2
        String id = createSubmittedAndApprove();

        // 用过期 version=0 归档 → 乐观锁冲突 (DB version=2)
        MvcResult result = performPost(API_PATH + "/" + id + "/archive", buildArchiveBody(0), mockUser("admin"));
    ...
    @Test
    @DisplayName("CT-7: 审计落库 (@Auditable ARCHIVE → sys_operation_log 有记录)")
    void testArchiveBizRequestAuditLog() throws Exception {
        String id = createSubmittedAndApprove();
        ...
        Integer auditCount = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM sys_operation_log " +
                        "WHERE biz_type = 'biz_request' AND operation_type = 'ARCHIVE' " +
                        "AND trace_id = ?",
                Integer.class, traceId);

        assertNotNull(auditCount, "审计日志查询不应返回 null");
        assertTrue(auditCount >= 1,
                "sys_operation_log 应至少有 1 条 ARCHIVE 审计记录 (traceId=" + traceId + "), 实际: " + auditCount);

        jdbcTemplate.query("SELECT biz_id, module, operation_type, biz_type FROM sys_operation_log " +
                        "WHERE biz_type = 'biz_request' AND operation_type = 'ARCHIVE' AND trace_id = ?",
                rs -> {
                    assertEquals(id, rs.getString("biz_id"), "审计记录 biz_id 应为归档的申请单 ID");
                    assertEquals("sample", rs.getString("module"), "审计记录 module 应为 sample");
                    assertEquals("ARCHIVE", rs.getString("operation_type"), "审计记录 operation_type 应为 ARCHIVE");
    ...
    void testArchiveBizRequestTraceIdPropagation() throws Exception {
        String id = createSubmittedAndApprove();
