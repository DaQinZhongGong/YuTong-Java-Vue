...
/**
 * GA2-L181 CT-deleteCustomer 契约测试。
 *
 * <p>设计来源: 58-后端API逐接口任务清单 (CT-deleteCustomer 验收标准)
 * <p>策略来源: operation-policies.yaml deleteCustomer → guardedDelete profile (natural-by-resource-state)
 * <p>契约来源: openapi.yaml DELETE /api/v1/customers/{id} (x-permission: biz:customer:delete, x-error-codes: [BIZ-404002, BIZ-409007])
 *
 * <p>覆盖矩阵:
 * <ol>
 *   <li>CT-1 成功删除 (200, code=0, 逻辑删除 deleted=true)</li>
 *   <li>CT-2 认证失败 (Mock 模式限制, @Disabled)</li>
 *   <li>CT-3 权限拒绝 (viewer 无 biz:customer:delete, 403 AUTH-403001)</li>
 *   <li>CT-4 客户不存在 (404 BIZ-404002)</li>
 *   <li>CT-5 已删除再删 (404 BIZ-404002, guardedDelete 自然幂等)</li>
 ...
 */
@DisplayName("CT-deleteCustomer: DELETE /api/v1/customers/{id} 契约测试")
class CTDeleteCustomerTest extends AbstractContractTest {
    ...
    private Customer buildValidCustomer() {
        ...
        c.setCustomerCode("CT-L181-D-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase());
        c.setCustomerName("CT删除测试客户");
    ...
    @Test
    @DisplayName("CT-1: 成功删除客户 (biz 用户, 200, code=0, 逻辑删除 deleted=true)")
    void testDeleteCustomerSuccess() throws Exception {
        ...
        Boolean deleted = jdbcTemplate.queryForObject(
                "SELECT deleted FROM biz_customer WHERE id = ?", Boolean.class, id);
        assertNotNull(deleted, "客户记录应存在 (物理查询)");
    ...
    void testDeleteCustomerUnauthenticated() {
    ...
    @Test
    @DisplayName("CT-3: 权限拒绝 (viewer 无 biz:customer:delete, 403 AUTH-403001)")
    void testDeleteCustomerForbidden() throws Exception {
        String id = createCustomerAndTrack();
        ...
        Boolean deleted = jdbcTemplate.queryForObject(
                "SELECT deleted FROM biz_customer WHERE id = ?", Boolean.class, id);
        assertNotNull(deleted, "客户记录应存在");
        assertFalse(deleted, "权限拒绝时客户不应被删除");
    }
    ...
    @Test
    @DisplayName("CT-5: 已删除再删 (guardedDelete 自然幂等, 404 BIZ-404002)")
    void testDeleteCustomerAlreadyDeleted() throws Exception {
        String id = createCustomerAndTrack();
        ...
        // 第二次删除: 已删除 → selectById 返回 null → 404 BIZ-404002
    ...
    void testDeleteCustomerAuditLog() throws Exception {
        String id = createCustomerAndTrack();
        ...
        Integer auditCount = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM sys_operation_log " +
                        "WHERE biz_type = 'biz_customer' AND operation_type = 'DELETE' " +
                        "AND trace_id = ?",
        ...
        jdbcTemplate.query("SELECT biz_id, module, operation_type, biz_type FROM sys_operation_log " +
                        "WHERE biz_type = 'biz_customer' AND operation_type = 'DELETE' AND trace_id = ?",
                rs -> {
                    assertEquals(id, rs.getString("biz_id"), "审计记录 biz_id 应为删除的客户 ID");
    ...
    void testDeleteCustomerTraceIdPropagation() throws Exception {
        String id = createCustomerAndTrack();
