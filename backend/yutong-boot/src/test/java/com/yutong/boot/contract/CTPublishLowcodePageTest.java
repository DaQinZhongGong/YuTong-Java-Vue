...
/**
 * CT-publishLowcodePage 契约测试 (operationId: publishLowcodePage)。
 *
 * <p>设计来源: 58-后端API逐接口任务清单 (CT-publishLowcodePage 验收标准)
 * <p>契约来源: routes.yaml web.lowcode.pages (permission: lc:page:list)
 *    + LcPageController POST /api/v1/lowcode/pages/{id}/publish
 *
 * <p>覆盖矩阵:
 * <ol>
 ...
 *   <li>CT-6 幂等发布 (已 PUBLISHED 再发布, 200 直接返回当前页面)</li>
 * </ol>
 *
 * <p>数据隔离: 每个用例独立创建测试数据 (UUID 后缀 pageCode), @AfterEach 物理清理。
 */
@DisplayName("CT-publishLowcodePage: POST /api/v1/lowcode/pages/{id}/publish 契约测试")
class CTPublishLowcodePageTest extends AbstractContractTest {
    ...
    private SaveLcPageRequest buildValidPageRequest() {
        ...
        req.setPageCode("CT-LC-PG-PB-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase());
        req.setPageName("CT发布测试页面");
    ...
    @Test
    @DisplayName("CT-1: 成功发布页面 (admin 用户, DRAFT→PUBLISHED, 200, code=0, status=PUBLISHED + traceId)")
    void testPublishLowcodePageSuccess() throws Exception {
        ...
        assertNotNull(data.get("publishedTime"), "发布后 publishedTime 不应为 null");
    ...
    void testPublishLowcodePageUnauthenticated() {
    ...
    @org.junit.jupiter.api.Disabled("LcPageController.publish 未加 @RequiresPermission(lc:page:publish), " +
            "权限码未在控制器层强制; 待补充注解后启用此用例")
    void testPublishLowcodePageForbidden() {
    ...
    void testPublishLowcodePageNotFound() throws Exception {
    ...
    void testPublishLowcodePageVersionMismatch() throws Exception {
        String[] idAndVersion = createPageAndTrack();
        String id = idAndVersion[0];
    ...
    @Test
    @DisplayName("CT-6: 幂等发布 (已 PUBLISHED 再发布, 200 直接返回当前页面)")
    void testPublishLowcodePageIdempotent() throws Exception {
        String[] idAndVersion = createPageAndTrack();
        String id = idAndVersion[0];
        int version = Integer.parseInt(idAndVersion[1]);
