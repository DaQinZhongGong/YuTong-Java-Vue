...
/**
 * CT-previewLowcodePage 契约测试 (operationId: previewLowcodePage)。
 *
 * <p>设计来源: 58-后端API逐接口任务清单 (CT-previewLowcodePage 验收标准)
 * <p>契约来源: routes.yaml web.lowcode.pages (permission: lc:page:list)
 *    + LcPageController POST /api/v1/lowcode/pages/{id}/preview
 *
 * <p>覆盖矩阵:
 * <ol>
 *   <li>CT-1 成功预览 (admin, 200, code=0, 返回 LcPageDetailVO + traceId)</li>
 *   <li>CT-2 认证失败 (Mock 模式限制, @Disabled)</li>
 *   <li>CT-3 权限拒绝 (LcPageController 未加 @RequiresPermission, @Disabled)</li>
 *   <li>CT-4 页面不存在 (404 SYS-404001)</li>
 *   <li>CT-5 预览返回子表 (components/actions 数组字段存在)</li>
 * </ol>
 *
 * <p>数据隔离: 每个用例独立创建测试数据 (UUID 后缀 pageCode), @AfterEach 物理清理。
 */
@DisplayName("CT-previewLowcodePage: POST /api/v1/lowcode/pages/{id}/preview 契约测试")
class CTPreviewLowcodePageTest extends AbstractContractTest {
    ...
    private SaveLcPageRequest buildValidPageRequest() {
        ...
        req.setPageCode("CT-LC-PG-PV-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase());
        req.setPageName("CT预览测试页面");
        req.setPageType("DETAIL");
        req.setLayoutJson("{\"type\":\"detail\"}");
    ...
    @Test
    @DisplayName("CT-1: 成功预览页面 (admin 用户, 200, code=0, 返回 LcPageDetailVO + traceId)")
    void testPreviewLowcodePageSuccess() throws Exception {
        String id = createPageAndTrack();

        MvcResult result = performPost(API_PATH + "/" + id + "/preview", null, mockUser("admin"));
        ...
        assertNotNull(data.get("layoutJson"), "layoutJson 不应为 null");
    }
    ...
    void testPreviewLowcodePageUnauthenticated() {
    ...
    @org.junit.jupiter.api.Disabled("LcPageController.preview 未加 @RequiresPermission(lc:page:preview), " +
            "权限码未在控制器层强制; 待补充注解后启用此用例")
    void testPreviewLowcodePageForbidden() {
    ...
    void testPreviewLowcodePageNotFound() throws Exception {
        String nonExistentId = "01NOTEXIST00000000000000CT";

        MvcResult result = performPost(API_PATH + "/" + nonExistentId + "/preview", null, mockUser("admin"));
    ...
    @Test
    @DisplayName("CT-5: 预览返回子表 (components/actions 数组字段存在)")
    void testPreviewLowcodePageWithChildren() throws Exception {
        String id = createPageAndTrack();
        ...
        assertSuccess(result);
        JsonNode data = parseResult(result).data();
        assertNotNull(data.get("components"), "预览应包含 components 字段");
        assertTrue(data.get("components").isArray(), "components 应为数组");
        assertNotNull(data.get("actions"), "预览应包含 actions 字段");
